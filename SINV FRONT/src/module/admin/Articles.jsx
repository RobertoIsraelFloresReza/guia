...
import { FaBox, FaSearch, FaToggleOn, FaToggleOff, FaEdit, FaPlus, FaEye, FaTag, FaInfoCircle } from 'react-icons/fa';
...
const Articles = () => {
    ...
    const getArticles = async () => {
        try {
            ...
            showErrorAlert('No se pudieron cargar los artículos');
        } finally {
            setLoading(false);
        }
    };
    ...
    const toggleStatus = async (article) => {
        const result = await Swal.fire({
            title: `¿Cambiar estado del artículo ${article.name}?`,
            text: `El artículo quedará como ${article.status ? 'inactivo' : 'activo'}`,
            icon: 'question',
            ...
            try {
                ...
                if (response.data) {
                    ...
                    Swal.fire({
                        ...
                        text: `El artículo ahora está ${!article.status ? 'activo' : 'inactivo'}`,
                        confirmButtonColor: '#00f0ff',
                        background: '#0f172a',
                        color: '#ffffff',
                    });
    ...
    const filteredArticles = articles.filter(article =>
        `${article.name} ${article.description} ${article.category?.name}`
            .toLowerCase()
    ...
    const handleArticleCreated = (newArticle) => {
        ...
        setExistingArticles(prev => [...prev, newArticle]);
    };
    ...
    return (
        <div className="min-h-screen bg-black w-full relative overflow-hidden">
                ...
                >
                    <div>
                        ...
                        <p className="text-cyan-300/80 flex items-center gap-2">
                            <FaBox className="text-cyan-400" /> Administra los artículos del sistema
                        </p>
                    </div>
                    ...
                    <div className="flex gap-3 items-center">
                        ...
                        >
                            ...
                            <input
                                type="text"
                                placeholder="Buscar artículo, descripción o categoría..."
                        ...
                        >
                            <FaPlus className="h-4 w-4" /> Nuevo Artículo
                        </motion.button>
                    </div>
                </motion.div>
                                ...
                                <table className="min-w-full divide-y divide-gray-700/50">
                                    <thead className="bg-gradient-to-r from-cyan-500/10 to-purple-500/10">
                                        <tr>
                                            ...
                                            <th className="px-6 py-4 text-left text-xs font-medium text-cyan-400 uppercase tracking-wider">Descripción</th>
                                            ...
                                            >
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap">
                                                    <motion.button
                                                        ...
                                                        className={`px-3 py-1.5 rounded-full text-xs font-semibold flex items-center gap-1.5 ${article.status
                            ...
                            {filteredArticles.length === 0 && !loading && (
                                <div className="text-center py-12">
                                    <div className="mx-auto w-20 h-20 rounded-full bg-gradient-to-r from-cyan-500/20 to-purple-500/20 flex items-center justify-center mb-4 shadow-lg shadow-cyan-500/20">
                                        <FaBox className="h-10 w-10 text-cyan-400" />
                                    </div>
                                    <h3 className="mt-4 text-xl font-medium text-gray-300">No se encontraron artículos</h3>
                                ...
                                <div className="space-y-6">
                                    <div className="flex items-start gap-6">
                                        <div className="w-24 h-24 rounded-xl bg-gradient-to-br from-cyan-600 to-purple-600 flex items-center justify-center text-white text-4xl font-bold shadow-lg">
                                            {selectedArticle.name?.charAt(0) || 'A'}
                                        </div>
                                        <div className="flex-1">
                                            <h4 className="text-2xl font-bold text-white">
                                                {selectedArticle.name || 'Sin nombre'}
                                            </h4>
                                            <div className="flex items-center gap-3 mt-2">
                                                <span className={`px-3 py-1 rounded-full text-xs font-semibold ${selectedArticle.status
                                                    ? 'bg-green-500/20 text-green-400 border border-green-500/30'
                                                    : 'bg-red-500/20 text-red-400 border border-red-500/30'
                                                    }`}>
                                                    {selectedArticle.status ? 'ACTIVO' : 'INACTIVO'}
                                                </span>
                                                <span className="px-3 py-1 rounded-full text-xs font-semibold bg-purple-500/20 text-purple-400 border border-purple-500/30">
                                                    {selectedArticle.category?.name || 'Sin categoría'}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 border-t border-gray-700/50">
                                        <div className="bg-gray-800/50 rounded-lg p-4 border border-gray-700/30">
                                            <h5 className="text-sm font-medium text-cyan-400 mb-3 flex items-center gap-2">
                                                <FaInfoCircle /> Información del Artículo
                                            </h5>
                                            <div className="space-y-3">
                                                <div>
                                                    <p className="text-xs text-gray-400">Nombre</p>
                                                    <p className="text-gray-200 font-medium">
                                                        {selectedArticle.name || 'No disponible'}
                                                    </p>
                                                </div>
                                                <div>
                                                    <p className="text-xs text-gray-400">Descripción</p>
                                                    <p className="text-gray-200 font-medium">
                                                        {selectedArticle.description || 'No disponible'}
                                                    </p>
                                                </div>
                                                <div>
                                                    <p className="text-xs text-gray-400">Estado</p>
                                                    <p className={`font-medium ${selectedArticle.status ? 'text-green-400' : 'text-red-400'
                                                        }`}>
                                                        {selectedArticle.status ? 'Activo' : 'Inactivo'}
                                                    </p>
                                                </div>
                                            ...
                                            {selectedArticle.category ? (
                                                <div className="space-y-3">
                                                    <div>
                                                        <p className="text-xs text-gray-400">Nombre</p>
                                                        <p className="text-gray-200 font-medium">
                                                            {selectedArticle.category.name || 'No disponible'}
                                                        </p>
                                                    </div>
                                                    <div>
                                                        <p className="text-xs text-gray-400">Estado</p>
                                                        <p className={`font-medium ${selectedArticle.category.status ? 'text-green-400' : 'text-red-400'
                                                            }`}>
                                                            {selectedArticle.category.status ? 'Activo' : 'Inactivo'}
                                                        </p>
                                                    </div>
                                                </div>
                                            ) : (
                                    ...
                                    <div className="pt-4 border-t border-gray-700/50 flex justify-end">
                                        <motion.button
                                            ...
                                            whileTap={{ scale: 0.97 }}
                                            onClick={() => {
                                                toggleStatus(selectedArticle);
                                                setShowDetailsModal(false);
                                            }}
                                            className={`px-4 py-2 rounded-lg flex items-center gap-2 ${selectedArticle.status
                                        ...
                                        >
                                            {selectedArticle.status ? (
                                                <>
                                                    <FaToggleOff /> Desactivar Artículo
                                                </>
                                            ) : (
                                                <>
                                                    <FaToggleOn /> Activar Artículo
                                                </>
                                            )}
                                        </motion.button>
                                    </div>
            ...
            <ArticleEditModal
                ...
                articleData={selectedArticleToEdit}
                existingArticles={existingArticles}
            />
