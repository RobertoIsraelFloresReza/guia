...
const StorageCreateModal = ({ isOpen, onClose, onStorageCreated, existingStorages }) => {
    ...
    const fetchCategories = async () => {
        try {
            ...
            if (response.status === "OK") {
                // Filtrar solo categorías habilitadas (status = true)
    ...
    const fetchResponsibles = async () => {
        try {
            ...
            if (response.status === "OK") {
                ...
                // Obtener IDs de responsables ya asignados
                const assignedResponsibleIds = existingStorages
                    .filter(storage => storage.responsible)
                    .map(storage => storage.responsible.id);
                
                // Filtrar responsables disponibles (no asignados) y con status habilitado
    ...
    const validateForm = () => {
        ...
        } else if (existingStorages.some(storage => 
            storage.identifier.toLowerCase() === formData.identifier.toLowerCase()
        )) {
            newErrors.identifier = 'Este identificador ya está en uso';
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await AxiosClient.post('/storage/', payload);
            ...
            if (response.status === "CREATED") {
                ...
                Swal.fire({
                    ...
                    text: `Almacén ${formData.identifier} registrado correctamente`,
                    confirmButtonColor: '#00f0ff',
                    background: '#0f172a',
                    color: '#ffffff',
                });
                
                onStorageCreated(completeStorage);
                handleClose();
            }
        } catch (error) {
                                    ...
                                    <div>
                                        ...
                                        >
                                            ...
                                            {categories.length > 0 ? (
                                                ...
                                                <option value="" disabled>
                                                    No hay categorías habilitadas disponibles
                                                </option>
                                            )}
                                        </select>
                                        {errors.categoryId && (
                                        ...
                                        {categories.length === 0 && !errors.categoryId && (
                                            <p className="mt-1 text-sm text-yellow-400">
                                                No hay categorías habilitadas. Por favor, active alguna categoría primero.
                                            </p>
                                        )}
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        {responsibles.length > 0 ? (
                                            ...
                                            <div className="text-sm text-gray-400 italic">
                                                No hay responsables disponibles (todos están asignados a otros almacenes)
                                            </div>
                                        )}
                                    </div>
